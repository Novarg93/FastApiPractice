...
function onPickAvatar(e: Event) {
    ...
    if (!/^image\//.test(file.type)) {
        profileErrors.value.avatar = 'Поддерживаются только изображения'
        return
    }
    if (file.size > 5 * 1024 * 1024) { // 5MB
        profileErrors.value.avatar = 'Файл слишком большой (макс. 5MB)'
        return
    }
...
async function saveProfile() {
    ...
    try {
        ...
        if (name.value !== (auth.user?.name ?? '')) {
            const { data } = await http.patch('/auth/me', { name: name.value }, { headers: auth.authHeader() })
            // обновляем стор
            auth.user = { ...auth.user!, name: data.name ?? name.value }
        }

        // 2) если выбран новый аватар — грузим multipart/form-data
        if (avatarFile.value) {
            const fd = new FormData()
            fd.append('file', avatarFile.value) // backend: File(...) param с именем 'file'
            const { data } = await http.post('/auth/avatar', fd, {
                headers: { ...auth.authHeader(), 'Content-Type': 'multipart/form-data' },
            })
            // ожидаем, что вернётся обновлённый user или хотя бы url
            const url = data?.avatar ?? data?.url ?? null
            if (url) {
                avatarUrl.value = url
                auth.user = { ...auth.user!, ...(url ? { avatar: url } : {}) }
            }
            // очистим превью
        ...
        // можно показать тост/уведомление — «Сохранено»
    } catch (e: any) {
...
async function deleteAccount() {
    ...
    try {
        await http.delete('/auth/delete-account', { headers: auth.authHeader() })
        await auth.logout()
        router.push('/login')
    } catch (e: any) {
        // можно показать тост
